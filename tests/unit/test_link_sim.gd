...
func test_100_m_avec_un_rouleau_de_114_3_mm_font_278_ticks() -> void:
	...
	assert_eq(LinkConst.ticks_for_metres(100.0), 278)
	assert_almost_eq(LinkConst.circumference_mm(), 359.08, 0.01)


func test_les_bornes_d_argument_firmware_de_docs_01() -> void:
	assert_true(LinkConst.is_valid_firmware_argument(1))
	assert_true(LinkConst.is_valid_firmware_argument(278))
	assert_true(LinkConst.is_valid_firmware_argument(32767))
	assert_false(LinkConst.is_valid_firmware_argument(0), "zero tick n'a pas de sens")
	assert_false(LinkConst.is_valid_firmware_argument(32768), "atoi remplit un int 16 bits")


# --- Handshake ---------------------------------------------------------------

func test_le_depart_n_est_autorise_qu_en_etat_identified() -> void:
	assert_eq(_sim.get_link_state(), LinkConst.State.DISCONNECTED)
	assert_false(_sim.can_start_race(), "au repos, START doit etre interdit")

	_sim.start()
	assert_eq(_sim.get_link_state(), LinkConst.State.PORT_OPEN)
	assert_false(_sim.can_start_race(), "port ouvert n'est PAS une preuve — la faute de la v1")

	_advance(0.5)
	assert_eq(_sim.get_link_state(), LinkConst.State.IDENTIFIED)
	assert_true(_sim.can_start_race())
	assert_eq(_sim.get_firmware_version(), LinkConst.FIRMWARE_VERSION)


func test_la_version_est_annoncee_par_une_trame() -> void:
	_connect_sim()
	var versions := _of_kind(LinkConst.Frame.VERSION)
...
func test_toute_duree_qui_ferait_terminer_le_firmware_est_refusee() -> void:
	...
	_connect_sim()
	assert_true(_sim.send_command(LinkConst.TIME_COMMAND), "t60 est la valeur sure")
...
func test_le_decompte_emet_cd_3_a_cd_0_a_une_seconde_d_intervalle() -> void:
	...
	_advance(0.9)
	assert_eq(_of_kind(LinkConst.Frame.COUNTDOWN).size(), 0, "rien avant 1000 ms")

	_advance(3.4)
	var values: Array = []
	for p: Dictionary in _of_kind(LinkConst.Frame.COUNTDOWN):
...
func test_course_distance_a_2_capteurs_les_riders_finissent_pas_la_course() -> void:
	...
	var finishes := _of_kind(LinkConst.Frame.RIDER_FINISH)
	...
	var before := _of_kind(LinkConst.Frame.PROGRESS).size()
	_advance(2.0)
	assert_gt(_of_kind(LinkConst.Frame.PROGRESS).size(), before,
...
func test_course_distance_a_4_capteurs_la_course_se_termine() -> void:
	...
	assert_eq(_of_kind(LinkConst.Frame.RIDER_FINISH).size(), 4)
	var before := _of_kind(LinkConst.Frame.PROGRESS).size()
	_advance(2.0)
	assert_eq(_of_kind(LinkConst.Frame.PROGRESS).size(), before,
...
func test_les_ticks_sont_cumules_et_jamais_decroissants() -> void:
	...
	_connect_sim()
	_sim.send_command("x")
	_sim.send_command(LinkConst.TIME_COMMAND)
	_sim.send_command("g")
	_advance(12.0)

	var frames := _of_kind(LinkConst.Frame.PROGRESS)
...
func test_le_mode_temps_ne_se_termine_jamais_de_lui_meme() -> void:
	...
	_sim.send_command("g")
	_advance(70.0)
	assert_eq(_of_kind(LinkConst.Frame.RIDER_FINISH).size(), 0)
...
func test_faux_depart_pendant_le_decompte() -> void:
	...
	_sim.send_command("g")
	_advance(2.0)
	var fs := _of_kind(LinkConst.Frame.FALSE_START)
	assert_eq(fs.size(), 1)
...
func test_tick_fantome_ajoute_un_tick_sans_mouvement() -> void:
	...
	_sim.send_command("g")
	_advance(6.0)
	var before: Array = _of_kind(LinkConst.Frame.PROGRESS).back()["ticks"]
	_sim.inject_phantom_tick(0)
	_advance(0.1)
	var after: Array = _of_kind(LinkConst.Frame.PROGRESS).back()["ticks"]
...
func test_trame_corrompue_remonte_en_unknown_sans_rien_casser() -> void:
	...
	var unknown := _of_kind(LinkConst.Frame.UNKNOWN)
...
func test_perte_et_retour_du_lien() -> void:
	...
	assert_eq(_sim.get_link_state(), LinkConst.State.LINK_LOST)
...
func test_a_graine_fixee_le_scenario_se_rejoue_a_l_identique() -> void:
	...
	for run: int in range(2):
		...
		sim.frame_received.connect(func(kind: int, payload: Dictionary) -> void:
			if kind == LinkConst.Frame.PROGRESS:
				ticks.append(payload["ticks"][0]))
		sim.start()
		for i: int in range(600):
			sim.advance(0.02)
		sim.send_command("x")
		sim.send_command(LinkConst.TIME_COMMAND)
		sim.send_command("g")
		for i: int in range(600):
			sim.advance(0.02)
		runs.append(ticks)
...
func test_chaque_etat_du_lien_est_atteint() -> void:
	...
	_sim.inject_link_return()
	_sim.stop()
	assert_has(_states, LinkConst.State.PORT_OPEN)
	assert_has(_states, LinkConst.State.IDENTIFIED)
	assert_has(_states, LinkConst.State.LINK_LOST)
	assert_has(_states, LinkConst.State.DISCONNECTED)
